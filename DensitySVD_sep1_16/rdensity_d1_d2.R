rdensity_d1_d2<-function(D,S,dimIndex,N,nBin=10,sampleSize=1,hyper,kk=0){
  ...
  if((dimIndex == 1) & ((S[1]-beta) > 0)) #### d_1 should be in d_2 to \infty
  {
    ...
    if(bFlag == 1){
      ...
      if(d1_near_mode_0 < 60){
        d1_near_mode_1 = NR_method_derivative_d1(D,S,dimIndex,N,max_iter=10,hyper) ##### replaced by NR method
        print_debug(paste0("d1_near_mode_1 ",d1_near_mode_1),hyper$debug)
      }
    }
    print(d1_near_mode_0)
    if(!((d1_near_mode_1 >= 100) || ((d1_near_mode_1+D[2]) > 150))){
      d_tmp0 = d_density_d1_log(d1_near_mode_0,D,S,dimIndex,N,hyper,bLog=1)
      d_tmp1 = d_density_d1_log(d1_near_mode_1,D,S,dimIndex,N,hyper,bLog=1)
    ...
    print_debug(paste0("d1_near_mode =  ",d1_near_mode),hyper$debug)
    ################
  ...
  for (i in  1:length(y)){
    ...
    if(bLog==1){
      ...
      val[i] = d_density_d1_log(x,D,S,dimIndex,N,hyper,bLog)
      ...
      val[i] = exp(d_density_d1_log(x,D,S,dimIndex,N,hyper,bLog))
    } 
...
d_density_d1_log<-function(y,D,S,dimIndex,N,hyper,bLog,approx=0){
