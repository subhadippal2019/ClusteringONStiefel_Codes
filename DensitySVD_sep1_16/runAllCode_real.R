...
#R CMD BATCH --no-save --no-restore '--args ./nec_175.Rdata 6' ./runAllCode_real.R ./run_real_nec_175.log
...
for(i in 4:n_clust_to_run){
  ...
  out_file_name = sprintf("../RealData/NEO/output/output_nclust_%d_realNEC_%d.RData",nClust,data_id)
  ...
  data_with_MCMC_sample = finiteMixtureML(data=nec_175,nClust,nClust,max_iter=1000,run_id=as.numeric(data_id),output_file = out_file_name)
