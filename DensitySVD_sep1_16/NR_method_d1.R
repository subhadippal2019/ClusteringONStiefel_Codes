NR_method_derivative_d1<-function(D,S,dimIndex,N,max_iter,hyper){
  ...
  #x = (1:4000)/10
  #out=d_density_d1_log(y=x,D,S,1,N,hyper,1)
  ...
  for (i in  1:max_iter){
    ...
    #print(paste0("iter: ",i," d1_new = ",d1_new)) 
  	...
  	dRet = 0.0
  	print_debug(paste0("eigenvalue = ",eigenValues," N = ",N," S = ",S),hyper$debug)
  	...
  	if((D[1] >= 100) || (D[2] >= 100) || ((D[1]+D[2]) > 150)){
  	  ...
  	  hyper0F1_val = approx_hyper(eigenValues)
  	}else{
  	  hyper0F1_val = .C("hyper_2by2_R",cc=1.5,eigenValues,dRet)[[3]]
  	  #log_f_d1_old = (alpha-1.0)*log(d1_old)+(d1_old*(S[dimIndex]-beta))- N * log(hyper0F1_val)
	    print_debug(paste0("0F1 val = ",hyper0F1_val),hyper$debug)
  	}
	  dRet = 0.0
	  ...
	  #hyper0F1_val_2 = .C("hyper_2by2_R",1.5,eigenValues2,dRet)[[3]]
	  ...
	  #D[dimIndex]=d1_old-0.001;
	  #eigenValues2=D^2/4;
	  #hyper0F1_val_3 = .C("hyper_2by2_R",1.5,eigenValues2,dRet)[[3]]
...
NR_method_function_d1<-function(D,S,dimIndex,N,max_iter,hyper,epsilon = 0.1){
